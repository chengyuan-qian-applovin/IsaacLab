...
class FleetMonitor:
    """Polls :func:`fetch_status` in a background thread and keeps the latest snapshot.

    For UIs that only observe the fleet (the desktop launcher and the headset
    app both show the server's progress table): they read :meth:`status` on
    their own schedule and never block on the network. Construct, :meth:`start`,
    call :meth:`poll_now` after the operator presses Connect, :meth:`stop` at
    exit. Thread-safe.
    """

    def __init__(self, server_url: str, token: str | None = None, interval_s: float = 15.0):
        ...
        self._polled_at = 0.0
        self._connected = False  # ever succeeded
    ...
    def poll_now(self) -> None:
        """Poll on the next loop iteration instead of waiting out the interval."""
        self._wake.set()

    def matches(self, server_url: str, token: str | None) -> bool:
        """Whether this monitor already watches ``server_url`` with ``token``."""
        return self.server_url == server_url.rstrip("/") and self.token == (token or None)

    def _loop(self) -> None:
        while not self._stop.is_set():
            try:
                snapshot = fetch_status(self.server_url, self.token)
                ...
                with self._lock:
                    self._snapshot, self._error, self._connected = snapshot, None, True
                    self._polled_at = time.time()
            self._wake.wait(timeout=self.interval_s)
            self._wake.clear()

    @property
    def scenes(self) -> dict[str, dict] | None:
        """Scene id -> server scene row from the latest good snapshot, or None before the first."""
        with self._lock:
            if self._snapshot is None:
                return None
            return {s["scene_id"]: s for s in self._snapshot["scenes"]}

    def status(self) -> dict:
        """JSON-ready summary: connection state, fleet totals, online collectors, and the scene rows."""
        with self._lock:
            snapshot, error, polled_at, connected = self._snapshot, self._error, self._polled_at, self._connected
        out = {
        ...
        if snapshot is not None:
            out["totals"] = snapshot["totals"]
            out["online"] = [c["collector_id"] for c in snapshot["collectors"] if c["online"]]
            out["scenes"] = {s["scene_id"]: s for s in snapshot["scenes"]}
        return out
